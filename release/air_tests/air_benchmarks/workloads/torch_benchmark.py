...
@click.option("--num_nodes", type=int, default=1)
def run(
    num_runs: int = 1,
    num_epochs: int = 4,
    num_workers: int = 4,
    ...
    output_dir = str(Path.home() / "torch_benchmark" / f"{'external' if custom_scheduler else 'standard'}--{current_datetime}{'--local' if local else '--np-' + str(num_nodes)}")
